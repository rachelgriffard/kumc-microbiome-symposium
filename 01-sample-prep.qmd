...
//| echo: false
{
  ...
  const samples = noise.map((n, i) => {
    ...
    return {
      batch, group,
      x: (batch === "Batch 1" ? -2.5 : 2.5) + n.nx * 0.85,
      y: (group === "Case"    ?  0.45 : -0.45) + n.ny * 0.85
    };
  });
  ...
  function makePlot(field, domain, range) {
    return Plot.plot({
      ...
      y: { label: "PC2", grid: true, tickFormat: d => d.toFixed(1) },
  ...
  const caption = design === "Confounded"
    ? "Confounded design: the two plots are mirror images — batch and biology cannot be separated."
    : "Balanced design: the batch effect is visible on the left, but cases and controls are mixed within each batch on the right.";
