...
Another way that people choose to deal with the unequal sampling depth problem is by normalizing their data in a multitude of different ways. Some of the more common options follow.
...
**CSS normalization** (cumulative sum scaling, metagenomeSeq) and **DESeq2 normalization** borrow from RNA-seq workflows and attempt to normalize without discarding data.

**CLR transformation** (centered log-ratio) is the current statistical recommendation for compositional data. It converts relative abundances into log-ratios with respect to the geometric mean, making the data more amenable to standard statistical tests.

The field does not have a consensus. For diversity analysis, rarefaction or relative abundance is most common. For differential abundance, normalization tools (ANCOM-BC, DESeq2, ALDEx2) are more commonly used.
