...
The result is that a taxon observed in 100 reads in a sample with 10,000 total reads (1%) looks very different from the same taxon in 500 reads in a sample with 50,000 reads (also 1%), even though the relative abundance is identical. Before we find a way to normalize, we cannot compare the absolute counts that we have after denoising.

Let's talk about some potential options for this issue.

## Rarefaction

The traditional approach is to rarefy, or subsample, all samples down to the same read depth. Sometimes this is the depth of the shallowest sample, or a chosen cutoff that retains most samples.
...
**Relative abundance** divides each cell by the sample's total reads. Simple, but proportional and therefore compositional: the relative abundance of one taxon affects all others.
...
The field does not have a consensus. For diversity analysis, rarefaction or relative abundance is most common. For differential abundance, normalization tools (ANCOM-BC, DESeq2, ALDEx2) are preferred.